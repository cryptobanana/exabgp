...
def check_fifo(name):
    try:
        if not stat.S_ISFIFO(os.stat(name).st_mode):
            sys.stdout.write(
                "error: a file exist which is not a named pipe (%s)\n"
        ...
        sys.stdout.write(
            "error: could not create the named pipe %s\n" % os.path.abspath(name)
        )
        return False
    except IOError:
        sys.stdout.write(
            "error: could not access/delete the named pipe %s\n" % os.path.abspath(name)
        )
        sys.stdout.flush()
    except socket.error:
        sys.stdout.write(
            "error: could not write on the named pipe %s\n" % os.path.abspath(name)
        )
        sys.stdout.flush()
...
def main(location=""):
    ...
    if not location:
        ...
        sys.stderr.write(
            "run with 'env exabgp_cli_pipe=<location>' if you are trying to mess with ExaBGP's intenals"
        )
        sys.stderr.flush()
        sys.exit(1)
